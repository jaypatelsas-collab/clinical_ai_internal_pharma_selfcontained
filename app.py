...
/* Sidebar */
[data-testid="stSidebar"]{background:linear-gradient(180deg,#830051 0%,#6F0045 100%);border-right:1px solid #65003F}
...
[data-testid="stSidebar"] [role="radiogroup"] label:has(input:checked){background:var(--white)!important;color:var(--mulberry)!important;box-shadow:0 3px 10px rgba(0,0,0,.16)}
[data-testid="stSidebar"] [role="radiogroup"] label:has(input:checked) p{color:var(--mulberry)!important}
...
[data-baseweb="popover"] [role="option"]:hover,[data-baseweb="menu"] [role="option"]:hover{background:#F3E5ED!important;color:var(--charcoal)!important}

/* Shared page header: no eyebrow/badge above title */
.hero{padding:20px 25px;border-radius:13px;background:var(--mulberry);color:var(--white);margin-bottom:16px;overflow:visible;box-shadow:0 8px 22px rgba(131,0,81,.16);border:1px solid rgba(255,255,255,.10)}
...
.kpi-card:before{content:"";position:absolute;left:0;top:0;right:0;height:4px;border-radius:12px 12px 0 0;background:var(--mulberry)}
.kpi-label{font-size:.78rem;font-weight:700;color:var(--charcoal);text-transform:uppercase;letter-spacing:.035em}
.kpi-value{font-size:1.72rem;font-weight:800;color:var(--mulberry);margin-top:10px;line-height:1}
...
.section-title{font-size:1.05rem;font-weight:800;color:var(--mulberry);margin-bottom:4px}
.section-subtitle{font-size:.84rem;color:var(--charcoal);margin-bottom:8px}
.action{border:1px solid var(--line);border-left:5px solid var(--mulberry);border-radius:10px;padding:11px 14px;margin:7px 0;background:var(--white);box-shadow:0 2px 8px rgba(63,68,68,.04)}
.red,.blue,.pink{border-left-color:var(--mulberry)}
.meta{font-size:.82rem;color:var(--charcoal)}
.update-badge{display:inline-block;padding:6px 10px;border:1px solid #D8C3CF;border-radius:999px;background:#F8F1F5;color:var(--mulberry);font-size:.76rem;font-weight:650;margin-bottom:8px}

/* Buttons and tables */
.stButton>button,.stDownloadButton>button{background:var(--mulberry)!important;color:var(--white)!important;border:1px solid var(--mulberry)!important;border-radius:8px!important;font-weight:700!important;box-shadow:none!important}
.stButton>button:hover,.stDownloadButton>button:hover{background:#690041!important;border-color:#690041!important;transform:none}
[data-testid="stDataFrame"] thead tr th{background:var(--mulberry)!important;color:var(--white)!important}
...
[data-testid="stMetric"]{border-top:3px solid var(--mulberry);border-radius:10px;padding:.5rem .7rem;background:var(--white)}
...
def polish_figure(fig, height=None):
    ...
    fig.update_layout(
        ...
        font=dict(color="#3F4444", family="Arial, sans-serif"),
        margin=dict(l=24, r=24, t=28, b=24),
        hoverlabel=dict(bgcolor="#FFFFFF", font_color="#3F4444", bordercolor="#D8C7D1"),
    )
    if height:
        fig.update_layout(height=height)
    fig.update_xaxes(gridcolor="#EEE8EB", zerolinecolor="#EEE8EB", linecolor="#D8D0D4")
    fig.update_yaxes(gridcolor="#EEE8EB", zerolinecolor="#EEE8EB", linecolor="#D8D0D4")
    return fig
