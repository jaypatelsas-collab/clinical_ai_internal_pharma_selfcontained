...
with st.sidebar:
    ...
    pages=['Executive Command Center','Initiative Registry','Value & Evidence','Governance & Risk','Adoption & Maturity','Incidents & Actions','Data Quality & Integrations','Admin Data Manager']
    page=st.radio('Navigate',pages)
    st.divider(); st.subheader('Score configuration')
    weights={'Efficiency':st.slider('Efficiency',0,50,20),'Quality':st.slider('Quality',0,50,25),'Adoption':st.slider('Adoption',0,50,15),'Compliance':st.slider('Risk & compliance',0,50,25),'Financial':st.slider('Financial value',0,50,15)}
    if sum(weights.values())==0: weights['Efficiency']=1

financial_score=np.clip(init['Validated_Annual_Benefit_USD'].div(init['Annual_Investment_USD']).mul(50),0,100)
init=init.copy(); init['AI_Value_Score']=((init.Efficiency_Score*weights['Efficiency']+init.Quality_Score*weights['Quality']+init.Adoption_Score*weights['Adoption']+init.Compliance_Score*weights['Compliance']+financial_score*weights['Financial'])/sum(weights.values())).round(1)
...
if page=='Executive Command Center':
    ...
    a,b,c=st.columns(3); a.warning(f'{len(high)} high-risk initiative(s) below compliance threshold'); b.info(f'{len(weak)} initiative(s) need stronger evidence'); c.error(f'{len(overdue)} overdue governance action(s)') if len(overdue) else c.success('No overdue governance actions')
    ...
    a,b,c,d=st.columns(4); a.metric('AI value score',f'{r.AI_Value_Score:.1f}/100'); b.metric('Validated net value',money(r.Validated_Net_Value)); c.metric('Evidence confidence',f'{r.Evidence_Confidence:.0f}%'); d.metric('Benefit status',r.Benefit_Status)
    c1,c2=st.columns(2)
    with c1:
