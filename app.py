...
def money(v): return f'${v/1_000_000:.2f}M' if abs(v)>=1_000_000 else f'${v/1_000:.0f}K'
def pct_series(s): return pd.to_numeric(s.astype(str).str.rstrip('%'),errors='coerce')
def workbook(d):
    b=io.BytesIO()
    with pd.ExcelWriter(b,engine='xlsxwriter') as w:
        for k,v in d.items(): v.to_excel(w,sheet_name=k[:31],index=False)
    return b.getvalue()
def validate(df,required,date_pairs=None):
    issues=[]
    miss=sorted(set(required)-set(df.columns))
    if miss: issues.append(('Critical',f'Missing columns: {", ".join(miss)}'))
    if df.empty: issues.append(('Critical','Dataset has no records.'))
    if df.duplicated().sum(): issues.append(('Warning',f'{df.duplicated().sum()} duplicate row(s).'))
    for s,e in date_pairs or []:
        if s in df and e in df:
            sd=pd.to_datetime(df[s],errors='coerce'); ed=pd.to_datetime(df[e],errors='coerce')
            if sd.isna().sum()+ed.isna().sum(): issues.append(('Warning',f'Invalid or missing dates detected in {s}/{e}.'))
            if (sd>ed).sum(): issues.append(('Critical',f'{(sd>ed).sum()} record(s) have {s} after {e}.'))
    return issues or [('Pass','No structural issues detected.')]

D=load_data(); init=D['initiatives']; controls=D['controls']; adoption=D['adoption']
...
if 'model_version' not in st.session_state: st.session_state.model_version='CAVF 1.0'
...
financial_score=np.clip(init['Validated_Annual_Benefit_USD'].div(init['Annual_Investment_USD'].replace(0,np.nan)).mul(50).fillna(0),0,100)
init=init.copy()
init['Financial_Score']=financial_score.round(1)
init['Weighted_Base_Score']=((init.Efficiency_Score*weights['Efficiency']+init.Quality_Score*weights['Quality']+init.Adoption_Score*weights['Adoption']+init.Compliance_Score*weights['Compliance']+init.Financial_Score*weights['Financial'])/max(sum(weights.values()),1)).round(1)
# Evidence confidence is retained as a separate transparency score and also moderates the final result.
init['Evidence_Adjustment']=((100-init.Evidence_Confidence)*0.10).round(1)
pen=st.session_state.risk_penalties
init['Risk_Adjustment']=0.0
init.loc[init.Risk_Level.eq('High'),'Risk_Adjustment']+=pen['High risk']
init.loc[init.Risk_Level.eq('Medium'),'Risk_Adjustment']+=pen['Medium risk']
init.loc[init.Compliance_Score.lt(70),'Risk_Adjustment']+=pen['Compliance below 70']
init.loc[init.Evidence_Confidence.lt(60),'Risk_Adjustment']+=pen['Evidence below 60']
critical_ids=set(D['incidents'].loc[(D['incidents'].Severity.eq('Critical')) & (~D['incidents'].Status.eq('Closed')),'Initiative_ID'].astype(str)) if {'Severity','Status','Initiative_ID'}.issubset(D['incidents'].columns) else set()
init.loc[init.Initiative_ID.astype(str).isin(critical_ids),'Risk_Adjustment']+=pen['Open critical incident']
init['AI_Value_Score']=np.clip(init['Weighted_Base_Score']-init['Evidence_Adjustment']-init['Risk_Adjustment'],0,100).round(1)
init['Validated_Net_Value']=init.Validated_Annual_Benefit_USD-init.Annual_Investment_USD
init['Evidence_Adjusted_Value']=init.Validated_Annual_Benefit_USD*init.Evidence_Confidence/100-init.Annual_Investment_USD
init['Scoring_Model_Version']=st.session_state.model_version
...
if page=='Executive Command Center':
    ...
    c1,c2=st.columns(2)
    with c1:
        cats=['Efficiency','Quality','Adoption','Compliance','Financial']; vals=[r.Efficiency_Score,r.Quality_Score,r.Adoption_Score,r.Compliance_Score,float(np.clip(r.Validated_Annual_Benefit_USD/r.Annual_Investment_USD*50,0,100))]
    ...
    with tab2:
        st.write('Evidence confidence indicates how reliable the supporting source is. The initiative file may provide an assessed confidence; these mappings are the recommended defaults for future connector data.')
    ...
    issues=validate(D['initiatives'],['Initiative_ID','Initiative','Department','Annual_Investment_USD','Validated_Annual_Benefit_USD'],[('Pilot_Start','Next_Review')])
    ...
    test=init
    if upload:
        try: test=pd.read_csv(upload) if upload.name.lower().endswith('.csv') else pd.read_excel(upload)
        except Exception as e: st.error(f'Unable to read file: {e}')
    for sev,msg in validate(test,['Initiative_ID','Initiative','Department','Annual_Investment_USD','Validated_Annual_Benefit_USD'],[('Pilot_Start','Next_Review')]):
        {'Pass':st.success,'Warning':st.warning,'Critical':st.error}[sev](msg)
    st.dataframe(test.head(30),use_container_width=True,hide_index=True)
    st.download_button('Download complete internal dummy-data workbook',workbook({k:v for k,v in D.items()}),'clinical_ai_internal_dummy_inputs.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
