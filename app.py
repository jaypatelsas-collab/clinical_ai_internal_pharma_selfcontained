...
st.markdown("""
<style>
:root{--navy:#2E1A47;--purple:#6B2D8F;--magenta:#B21F67;--orange:#F26B38;--muted:#6B6475;--line:#E1D9E8}
.block-container{padding-top:.75rem;padding-bottom:1.5rem;max-width:100%}
[data-testid="stSidebar"]{background:linear-gradient(180deg,#2E1A47 0%,#51246D 52%,#7A2C83 100%);border-right:1px solid rgba(255,255,255,.16)}
...
[data-testid="stSidebar"] [data-baseweb="select"] span,[data-testid="stSidebar"] [data-baseweb="select"] input,[data-testid="stSidebar"] [data-baseweb="select"] svg{color:#2E1A47!important;fill:#2E1A47!important;-webkit-text-fill-color:#2E1A47!important}
...
[data-baseweb="popover"] [role="option"]:hover,[data-baseweb="menu"] [role="option"]:hover{background:#F4ECF8!important}
.hero{padding:18px 24px;border-radius:14px;background:linear-gradient(118deg,#2E1A47 0%,#6B2D8F 58%,#B21F67 100%);color:white;margin-bottom:14px;overflow:visible;box-shadow:0 8px 24px rgba(76,35,99,.20);border-top:4px solid #F26B38}
.hero h1{margin:0;max-width:100%;font-size:clamp(1.45rem,2.2vw,2rem);line-height:1.22;white-space:normal!important;overflow:visible!important;text-overflow:clip!important;word-break:normal}.hero p{margin:.5rem 0 0;color:#F8EEF7}
.brand{display:flex;align-items:center;gap:10px;margin-bottom:8px}.brandmark{width:34px;height:34px;border-radius:10px;background:#F26B38;color:#2E1A47;display:flex;align-items:center;justify-content:center;font-weight:800}.brandname{font-size:1rem;font-weight:700;letter-spacing:.02em}
.badge{font-size:.69rem;letter-spacing:.1em;text-transform:uppercase;font-weight:700;color:#F9EAF5;margin-bottom:8px}
.kpi-grid{display:grid;grid-template-columns:repeat(4,minmax(170px,1fr));gap:12px;margin:8px 0 12px}.kpi-card{position:relative;background:#fff;border:1px solid var(--line);border-radius:13px;padding:15px 16px 14px;box-shadow:0 5px 16px rgba(15,23,42,.055);min-height:116px}.kpi-card:before{content:"";position:absolute;left:0;top:0;right:0;height:4px;border-radius:13px 13px 0 0;background:linear-gradient(90deg,#F26B38,#B21F67,#6B2D8F)}.kpi-label{font-size:.78rem;font-weight:700;color:#526176;text-transform:uppercase;letter-spacing:.035em}.kpi-value{font-size:1.72rem;font-weight:800;color:var(--navy);margin-top:10px;line-height:1}.kpi-note{font-size:.75rem;color:#718096;margin-top:9px}
...
.insight{display:flex;gap:10px;align-items:flex-start;padding:11px 0;border-bottom:1px solid #edf2f7}.insight:last-child{border-bottom:none}.insight-icon{width:28px;height:28px;border-radius:8px;background:#F4ECF8;color:#6B2D8F;display:flex;align-items:center;justify-content:center;font-weight:800}.insight-copy b{display:block;color:#183047;font-size:.9rem}.insight-copy span{color:#68778b;font-size:.78rem}
...
.update-badge{display:inline-block;padding:6px 10px;border:1px solid #cfe5e7;border-radius:999px;background:#F8F1FA;color:#6B2D8F;font-size:.76rem;font-weight:650;margin-bottom:8px}

[data-testid="stSidebar"] [role="radiogroup"] label:has(input:checked){background:linear-gradient(90deg,rgba(242,107,56,.95),rgba(178,31,103,.92))!important;color:#fff!important;box-shadow:0 4px 12px rgba(46,26,71,.25)}
.stButton>button,.stDownloadButton>button{background:linear-gradient(90deg,#6B2D8F,#B21F67)!important;color:#fff!important;border:0!important;border-radius:9px!important;font-weight:700!important;box-shadow:0 4px 10px rgba(107,45,143,.18)!important}
.stButton>button:hover,.stDownloadButton>button:hover{background:linear-gradient(90deg,#5A2479,#991A59)!important;transform:translateY(-1px)}
[data-testid="stDataFrame"] thead tr th{background:#2E1A47!important;color:#fff!important}
[data-testid="stDataFrame"]{border:1px solid #E1D9E8;border-radius:12px;overflow:hidden}
[data-testid="stMetric"]{border-top:3px solid #B21F67;border-radius:10px;padding:.5rem .7rem;background:#fff}
hr{border-color:#E1D9E8!important}
...
def render_page_header(section_label="Value & Governance Module", subtitle="Evidence-based portfolio oversight for value, quality, adoption, risk and responsible scale."):
    st.markdown(
        f'<div class="hero"><div class="badge">{section_label}</div><h1>Clinical AI Value Governance Platform</h1><p>{subtitle}</p></div>',
        unsafe_allow_html=True,
    )
...
if page == "Executive Command Center":
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Compliance score", gridcolor="#edf2f7", range=[35, 102]),
        yaxis=dict(title="AI value score", gridcolor="#edf2f7", range=[0, 105]),
    )
